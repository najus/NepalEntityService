...
from nes2.cli.migrate import migrate  # noqa: E402
...
def start(host, port, workers):
    ...
    uvicorn.run(
        "nes2.api.app:app", host=host, port=port, workers=workers, log_level="info"
    )
...
def dev(host, port):
    ...
    uvicorn.run("nes2.api.app:app", host=host, port=port, reload=True, log_level="info")
